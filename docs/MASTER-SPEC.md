...
| AS-6 | All imagery in v1 is the inline SVG icon system + CSS treatments + kanji watermarks (no photo uploads). Real photography is FUT-4 (upload pipeline specified in §6.10). | Media library with uploads in v1. | No file-storage dependency exists yet; keeps v1 shippable. |
