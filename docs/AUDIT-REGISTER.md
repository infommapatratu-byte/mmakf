...
| W2A-6 | **P0** | `src/db/schema.ts`, `src/db/federation.ts` | **Concurrent promotion corrupted the rank ledger.** Current rank is derived from `status = 'active'`, so "exactly one active rank per person per kind" is the invariant the entire grading record rests on. `awardRank()` superseded then inserted, so two simultaneous promotions both read "no active rank" and both inserted — leaving **two active ranks**, making a member's current grade ambiguous. | Attack "two simultaneous awards cannot leave two active ranks": expected 1 active, **got 2** | Two layers. (a) Partial unique index `rank_records_one_active_uk ON (person_id, kind) WHERE status='active'` makes the state unrepresentable even if application code is bypassed entirely. (b) `awardRank()` detects the unique violation through the driver's cause chain and retries the supersede-then-insert, bounded at 8 attempts — so the losing award still lands and history keeps **every** event, which is the correct append-only outcome. | **FIXED — VERIFIED** (4 tests incl. a 5-way race and a raw insert bypassing the application) |
