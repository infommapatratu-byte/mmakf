...
export const DYNAMIC_ROUTE_POLICY: Record<string, string> = {
  ...
  '/shop/brand/[slug]': 'NOT expanded, and the page says why itself: listings.brandId is a CLAIM the seller made, not a verified authorisation, and the two are routinely different — a reseller may lawfully stock branded goods without being an authorised dealer. Every row on the page still comes through publicListingPredicate(), so nothing unapproved is shown; what would be wrong is the ADVERTISING. A federation-hosted URL at mmakf.in/shop/brand/<trademark>, offered to search engines by the federation itself, reads as the federation vouching for a relationship it has not checked, and a trademark holder is entitled to read it that way. It is also the thin-aggregation shape that /shop/seller/[slug] guards against by requiring at least one live listing, except that here the harm is not a weak page but a claim about somebody else. The pages stay reachable and linked from the listings that carry the brand, which is the same position /athlete/[id] holds: a lookup that works, not a bulk crawl the federation never decided to publish.',
  '/shop/seller/[slug]':
